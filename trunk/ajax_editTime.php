<?php
	...
	$end = split(',',$etime);
		...
		$query = "select version,sequence,start_time,start_time_fraction,end_time,end_time_fraction,estart_time,estart_time_fraction,eend_time,eend_time_fraction,text from subs where subID=$id and lang_id=$lang and fversion=$fversion and edited_seq=$seq order by version DESC limit 1";
		...
		$oldtext = mysql_result($result, 0,10);
		...
		$query .= " values($id,$original_seq,$autorID,$myversion,0,0,NOW(),'$start_time',$start_time_fraction,'$end_time',$end_time_fraction,'$oldtext',$lang,$seq,1,'".$start[0]."',".$start[1].",'".$end[0]."',".$end[1].",$fversion)";
		mysql_query($query);
	...
	echo '<font color="blue">'.$start[0].','.$start[1].' --> '.$end[0].','.$end[1].'</font>';
